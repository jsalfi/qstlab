...
for d=1:1:N
    ...
    % load the KL solver results as VE [d] and the electric field files as
    % E_field [d]
    ...
    % This section plots and saves the hole pdf
    figure;
...
% end
% E_avg_x(d)=sum(abs(psi_new_x));
% E_avg_y(d)=sum(abs(psi_new_y));
% E_avg_z(d)=sum(abs(psi_new_z));
% 
% 
...
figure;
plot(1:iters,qs);
xlabel('Gate Voltage Applied(V)');
ylabel('Qubit splitting (meV)');
title("Zeeman Splitting as a function of gate voltage")
